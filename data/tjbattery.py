...
class TJBattery:
  def __init__(self, path=""):
    ...
    self.battery_name = file_name.split(".")[0]
  ...
  def _check(self, cycle=None, variable=None):
    ...
    if cycle is not None:
      if cycle not in self.cycle_index:
        raise ValueError(
          "cycle should be in [{},{}]".format(
            int(self.cycle_index.min()), int(self.cycle_index.max())
          )
        )
  ...
  def get_CV_stage(self, cycle, current_range=None):
    ...
    if current_range is not None:
      CV_df = CV_df[
        CV_df["<I>/mA"].between(np.min(current_range), np.max(current_range))
      ]
    return CV_df
  
  def get_rel_voltage(self, cycle):
  ...
  def plot_one_cycle_CCCV(self, cycle):
    ...
    ax[2].plot(
      CC_df["time/s"].values, CC_df["Q charge/mA.h"].values, color="b", linewidth=2
    )
    ax[2].plot(
      CV_df["time/s"].values, CV_df["Q charge/mA.h"].values, color="r", linewidth=2
    )
