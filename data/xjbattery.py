...
class Battery:
  ...
  def get_degradation_trajectory(self):
    ...
    return (
      degradation_trajectory  # ,(np.array(test_capacity_cycles),test_capacity)
    )
  ...
  def get_partial_value(self, cycle, variable, stage=1):
    ...
    relative_time_min = self.get_value(
      cycle=cycle, variable="relative_time_min"
    )
  ...
  def get_CC_value(self, cycle, variable, voltage_range=None):
    ...
    if voltage_range is None:
      ...
      index = np.where(
        (voltage >= voltage_range[0]) & (voltage <= voltage_range[1])
      )[0]
    value = value[index]
    return value
  ...
  def get_CV_value(self, cycle, variable, current_range=None):
    ...
    if current_range is None:
      ...
      index = np.where(
        (current >= np.min(current_range)) & (current <= np.max(current_range))
      )[0]
    value = value[index]
    return value
  ...
  def get_one_cycle(self, cycle):
    ...
    assert cycle <= self.data.shape[1]
    cycle_data = {}
    cycle_data["system_time"] = self.get_value(
      cycle=cycle, variable="system_time"
    )
    cycle_data["relative_time_min"] = self.get_value(
      cycle=cycle, variable="relative_time_min"
    )
    ...
    cycle_data["capacity_Ah"] = self.get_value(
      cycle=cycle, variable="capacity_Ah"
    )
    cycle_data["power_Wh"] = self.get_value(cycle=cycle, variable="power_Wh")
    cycle_data["temperature_C"] = self.get_value(
      cycle=cycle, variable="temperature_C"
    )
    cycle_data["description"] = self.get_value(
      cycle=cycle, variable="description"
    )
    return cycle_data
  ...
  def get_IC_curve1(self, cycle, voltage_range=[3.6, 4.19], step_len=0.01):
    ...
    """
    Q = self.get_CC_value(
      cycle=cycle, variable="capacity_Ah", voltage_range=voltage_range
    )
    V = self.get_CC_value(
      cycle=cycle, variable="voltage_V", voltage_range=voltage_range
    )
  ...
  def get_IC_curve2(self, cycle, voltage_range=[3.6, 4.19], step_len=0.01):
    ...
    """
    t = self.get_CC_value(
      cycle=cycle, variable="relative_time_min", voltage_range=voltage_range
    )
    V = self.get_CC_value(
      cycle=cycle, variable="voltage_V", voltage_range=voltage_range
    )
    I = self.get_CC_value(
      cycle=cycle, variable="current_A", voltage_range=voltage_range
    )
